...
namespace CraftCpp {
template <typename VertexType> OpenglBuffer<VertexType>::OpenglBuffer() {
...
void OpenglBuffer<VertexType>::setVaoAttributes() const {
  ...
  for (int a_dim : attributesDimensions) {
    if (a_dim != 0) {
      glVertexAttribPointer(i++, a_dim, GL_FLOAT, GL_FALSE, STRIDE,
                            (GLvoid *)offset);
...
template <typename VertexType>
void OpenglBuffer<VertexType>::storeData(
    const std::vector<VertexType> &buffer) const {
  if (!VBO || !VAO) {
    return;
  }
...
template class OpenglBuffer<BlockVertex>;
} // namespace CraftCpp