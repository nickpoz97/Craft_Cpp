...
int GameView::getWidth() const { return width; }

int GameView::getHeight() const { return height; }

int GameView::getScale() const { return scale; }
...
void GameView::setFov(int fov_degrees) { fov = fov_degrees; }

void GameView::setOrtho(int ortho_size) { ortho = ortho_size; }

int GameView::getFov() const { return fov; }
...
glm::mat4 GameView::getProjMatrix(GameView::ProjType pt) const {
  ...
  case ProjType::UI:
    return glm::ortho(0.0f, static_cast<float>(width), 0.0f,
...
int GameView::getOrtho() const { return ortho; }

float GameView::getRatio() const { return static_cast<float>(width) / height; }

bool GameView::isInstantiated() { return actualInstance; }
...
GameView *GameView::getActualInstance() { return actualInstance; }

std::unique_ptr<GameView> GameView::setInstance(int width, int height,
                                                float fov) {
  if (!isInstantiated()) {
