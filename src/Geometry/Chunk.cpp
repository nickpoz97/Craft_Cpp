...
void Chunk::computeChunkGeometry() const {
  ...
  // each visible face has INDICES_FACE_COUNT indices that represent the
  // triangle
  ...
  for (const auto &kv : blockMap) {
    ...
    // generate geometry of actual block (value returned by function is first
    // free position in buffer)
    v_it = generateBlockGeometry(v_it, block_pos, tileBlock,
                                 getVisibleFaces(kv.second, kv.first));
  }
#ifndef NDEBUG
...
Chunk::generateBlockGeometry(BufferType::iterator vertexIt,
  ...
  Cube cube{tileBlock.getBlockType(), visibleFaces, blockPos, vertexIt,
            getLightObstacles(blockPos)};
  return cube.end();
}
  ...
  for (int dx = 0; dx < Chunk::SIZE; dx++) {
    for (int dz = 0; dz < Chunk::SIZE; dz++) {
      ...
      if (w == BlockType::GRASS) {
        ...
        bool ok =
            SHOW_TREES && !(dx - 4 < 0 || dz - 4 < 0 || dx + 4 >= Chunk::SIZE ||
                            dz + 4 >= Chunk::SIZE);
...
std::array<glm::ivec3, 8> Chunk::getChunkBoundaries() const {
  ...
  it = std::transform(xzBoundaries.begin(), xzBoundaries.end(), it,
                      [](const glm::ivec2 &v) {
                        return glm::ivec3{v[0], 0, v[1]};
                      });
  it = std::transform(xzBoundaries.begin(), xzBoundaries.end(), it,
                      [](const glm::ivec2 &v) {
                        return glm::ivec3{v[0], Y_LIMIT, v[1]};
                      });
...
glm::ivec2 Chunk::chunked(const glm::vec3 &position) {
  return {chunked(static_cast<int>(position.x)),
...
glm::ivec2 Chunk::getMinXz(const glm::ivec2 &pq) { return pq * SIZE - 1; }

glm::ivec2 Chunk::getMaxXz(const glm::ivec2 &pq) {
  return getMinXz(pq) + SIZE + 1;
}

int Chunk::getMinX() const { return xzBoundaries[0][0]; }

int Chunk::getMinZ() const { return xzBoundaries[0][1]; }

int Chunk::getMaxX() const { return xzBoundaries[3][0]; }

int Chunk::getMaxZ() const { return xzBoundaries[3][1]; }

bool Chunk::checkBorder(const glm::ivec3 &pos,
                        const ::glm::ivec3 &direction) const {
  const glm::ivec3 new_pos = pos + direction;
  return new_pos.x <= getMinX() || new_pos.x >= getMaxX() ||
         new_pos.z <= getMinZ() || new_pos.z >= getMaxZ();
}

std::array<bool, 6> Chunk::getVisibleFaces(TileBlock w,
  ...
  return {
      ...
      w == BlockType::CLOUD ||
          w == BlockType::LEAVES, // pos.y > 0 && TileBlock{get_block(pos +
                                  // offsets[3])}.is_transparent(),
...
Chunk::getLightObstacles(const glm::ivec3 &blockPos) const {
  ...
  for (int ox = -1; ox <= 1; ++ox) {
    for (int oy = -1; oy <= 1; ++oy) {
      for (int oz = -1; oz <= 1; ++oz) {
        ...
        lightObstacles.emplace(
            offset, !TileBlock{getBlock(blockPos + offset)}.isTransparent());
      }
    }
  }
