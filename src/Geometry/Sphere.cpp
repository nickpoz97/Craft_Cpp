...
namespace CraftCpp {
decltype(Sphere::triangles_indices) Sphere::triangles_indices{{{4, 3, 0},
                                                               {1, 4, 0},
                                                               {3, 4, 5},
                                                               {4, 1, 5},
                                                               {0, 3, 2},
...
Sphere::Sphere(float r, int detail) : SuperClass{}, r{r} {
  ...
  for (const auto &ti : triangles_indices) {
    const std::array<glm::vec3, 3> actual_pos{
        positions[ti[0]], positions[ti[1]], positions[ti[2]]};
    const std::array<glm::vec2, 3> actual_uvs{uvs[ti[0]], uvs[ti[1]],
                                              uvs[ti[2]]};
...
Sphere::iterator_type Sphere::rec_gen_sphere(
...
Sphere::iterator_type
Sphere::fill_local_buffer(const std::array<glm::vec3, 3> &actual_pos_triangle,
