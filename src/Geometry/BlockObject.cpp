...
namespace CraftCpp {
template <> const float BlockObject<6>::A = 0.0 + 1 / 2048.0;

template <> const float BlockObject<6>::B = S - 1 / 2048.0;

template <> const float BlockObject<4>::A = 0.0;

template <> const float BlockObject<4>::B = S;

template <unsigned n_faces>
BlockObject<n_faces>::BlockObject(
    const BlockType &blockType, const std::array<bool, 6> &visibleFaces,
    ...
    : beginIterator{verticesIt}, nVertices{static_cast<size_t>(
                                     std::accumulate(visibleFaces.begin(),
                                                     visibleFaces.end(), 0) *
                                     INDICES_FACE_COUNT)} {
  ...
  for (auto face_ind_it{indices.begin()}; face_ind_it != indices.end();
       ++face_ind_it) {
    ...
    for (int i : *face_ind_it) {
      ...
      actual_vertex.uv = {du + (face_uvs[i].x ? B : A),
                          dv + (face_uvs[i].y ? B : A)};
#ifndef NDEBUG
      ...
      if (!lightObstacles.empty()) {
        ...
        if (directions.x != 0)
          obstaclesCoords.push_front(face_normal +
                                     glm::vec3{directions.x, 0, 0});
        if (directions.y != 0)
          obstaclesCoords.push_front(face_normal +
                                     glm::vec3{0, directions.y, 0});
        if (directions.z != 0)
          obstaclesCoords.push_front(face_normal +
                                     glm::vec3{0, 0, directions.z});
        if (directions.x != 0 && directions.z != 0) {
          obstaclesCoords.push_front(face_normal +
                                     glm::vec3{directions.x, 0, directions.z});
        }
        for (const auto &aoCoord : obstaclesCoords) {
          actual_vertex.ao +=
              actual_vertex.ao *
              lightObstacles.at(static_cast<glm::ivec3>(aoCoord));
        }
      }
...
Cube::Cube(const BlockType &blockType, const std::array<bool, 6> &visibleFaces,
...
template class BlockObject<6>;

template class BlockObject<4>;

Plant::Plant(const BlockType &blockType, const glm::vec3 &centerPosition,
             float asy_rotation, BlockVertexIterator verticesIt,
             const std::unordered_map<glm::ivec3, bool> &lightObstacles)
    : super{blockType,    {1, 1, 0, 0, 1, 1}, centerPosition,
            asy_rotation, lightObstacles,     verticesIt} {}
} // namespace CraftCpp