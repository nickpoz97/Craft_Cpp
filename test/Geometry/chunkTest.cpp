...
TEST_CASE("Chunk instantiation (not initialized) and block insertion") {
  ...
  c.setBlock(obstacle_block_pos + glm::ivec3{2, 0, 0},
             CraftCpp::BlockType::CLOUD);
...
TEST_CASE("Visibility formula") {
  ...
  auto p = glm::perspective(45.0f, 800.f / 600.f, .5f, 2.f);
  auto v = glm::lookAt(
      glm::vec3{0.f, 0.f, 0.f},
      // glm::vec3{(c.get_min_x() + c.get_max_x()) / 2,c.getHighestBlock() /
      // 2,(c.get_min_z() + c.get_max_z()) / 2},
      inner, glm::vec3{0.f, 1.f, 0.f});
  ...
  REQUIRE((glm::abs(clip_inner.x) < 1 && glm::abs(clip_inner.y) < 1 &&
           glm::abs(clip_inner.z) < 1));
  REQUIRE((clip_up_outer.y > 1 && glm::abs(clip_up_outer.x) < 1 &&
           glm::abs(clip_up_outer.z) < 1));
  REQUIRE((clip_down_outer.y < -1 && glm::abs(clip_down_outer.x) < 1 &&
           glm::abs(clip_down_outer.z) < 1));
  REQUIRE((clip_left_outer.x < -1 && glm::abs(clip_left_outer.y) < 1 &&
           glm::abs(clip_left_outer.z) < 1));
  REQUIRE((clip_right_outer.x > 1 && glm::abs(clip_right_outer.y) < 1 &&
           glm::abs(clip_right_outer.z) < 1));
}
