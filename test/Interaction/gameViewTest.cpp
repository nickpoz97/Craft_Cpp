...
TEST_CASE("Set GameView instance, attributes and destructor"){
...

    {
        ...
        auto mpCompare = glm::perspective(static_cast<float>(
            glm::radians(static_cast<float>(gameView->getFov()))), gameView->getRatio(), GameView::z_near, GameView::z_far
        );
        REQUIRE(
            glm::all(glm::equal(mp, mpCompare, 0.01f))
        );
        auto mu = gameView->getProjMatrix(GameView::ProjType::UI);
        auto muCompare = glm::ortho(0.0f, static_cast<float>(width), 0.0f, static_cast<float>(height), -1.0f, 1.0f);
        REQUIRE(
            glm::all(glm::equal(mu, muCompare, 0.01f))
        );
    }
    REQUIRE(!GameView::isInstantiated());
}
