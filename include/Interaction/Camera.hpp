...
public:
   ...
   * @param[in] rotation 2D vector consisting of 2 euler angles (yaw, pitch)
   * that defines camera direction
   * @param[in] camSpeed Camera object translation speed
   */
  Camera(const glm::vec3 &camPos, const glm::vec2 &rotation,
         float camSpeed = 10);
  Camera() = delete;
  ...
  /**
   * @brief translates the camera upward (following y-axis) by cameraSpeed
   * quantity
