...
public:
     ...
     * @note xz corners are computed with Chunk::computeXZBoundaries (these are in block-coordinate system)
     */
    ...
    /**
     * @brief get light obstacles for a block-coordinate (used for Ambient Occlusion)
     * @param[in] blockPos block-coordinate to test
     * @return map where key is the block-coordinate offset wrt input block-coordinate and
     * value is a flag that is true iff there is a opaque block in blockPos + offset position
     */
    ...
    /**
     * @brief check if specified block-coordinate is on Chunk object border or outside
     * @param[in] pos block-coordinate to be tested
     * @return true iff specified block-coordinate is on Chunk object border or outside
     */
