...
/// @note this indices will be used by BlockObject() to obtain the right uv coordinate from texture file
struct TileCube {
    /**
...
public:
    ...
    */
     ...
     * @return true iff BlockType in the range between BlockType::GRASS (included) and BlockType::BLUE_FLOWER (included)
     */
    [[nodiscard]] bool isPlant() const;

    /**
     * @brief check if this BlockType is one of the obstacles
     * @return true iff BlockType is not BlockType::EMPTY or BlockType::CLOUD or is not a plant
     */
    [[nodiscard]] bool isObstacle() const;

    /**
     * @brief Check if this BlockType blocks light (and casts shadows) or not
     * @return true iff it a plant or BlockType::GLASS or BlockType::LEAVES or BlockType::EMPTY
     */
     ...
     * @note this is used when you have to set a Block in a Chunk (you delete it instead of inserting an empty block)
     */
